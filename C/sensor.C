...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    double mediaTemperatura = 0.0, somaTemperatura = 0.0;
    double mediaUmidade = 0.0, somaUmidade = 0.0;
    ...
    {   
        ...
        somaPressao += comp_data.pressure/100;
        qtd += 1;

        t = clock() - t;
        
        double diff = (((double)t)/CLOCKS_PER_SEC)+1.0;
        clockTotal += diff;

        print_sensor_data(&comp_data);

        if (clockTotal >= 10.0) {
            mediaTemperatura = somaTemperatura/qtd;
